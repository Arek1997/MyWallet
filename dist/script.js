...
const checkForm = function () {
    if (letter.test(inputName.value) &&
        inputAmount.value !== '' &&
...
const saveTransaction = function () {
    ...
    if (inputCategory.value === 'food')
        icon = '<i class="fas fa-hamburger"></i>';
    if (inputCategory.value === 'cinema')
        icon = '<i class="fas fa-film"></i>';
    const html = `
                <div class="transaction">
                    <p class="transaction-name">${icon}${inputName.value}</p>
                    <p class="transaction-amount">${(+inputAmount.value).toFixed(2)}zł <button class="delete"><i class="fas fa-times"></i></button></p>
                </div>
    `;
